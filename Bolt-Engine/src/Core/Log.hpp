...
#define BT_ASSERT_MESSAGE_BOX (!BT_DIST && BT_PLATFORM_WINDOWS && false)

#if BT_ASSERT_MESSAGE_BOX
...
#define BT_CORE_FATAL_TAG(tag, ...) ::Bolt::Log::PrintMessageTag(::Bolt::Log::Type::Core, ::Bolt::Log::Level::Fatal, tag, __VA_ARGS__)
...
#define BT_FATAL_TAG(tag, ...) ::Bolt::Log::PrintMessageTag(::Bolt::Log::Type::Client, ::Bolt::Log::Level::Fatal, tag, __VA_ARGS__)
...
#define BT_CORE_FATAL(...)  ::Bolt::Log::PrintMessage(::Bolt::Log::Type::Core, ::Bolt::Log::Level::Fatal, __VA_ARGS__)
...
#define BT_FATAL(...)   ::Bolt::Log::PrintMessage(::Bolt::Log::Type::Client, ::Bolt::Log::Level::Fatal, __VA_ARGS__)

// Editor Console Logging Macros
#define BT_CONSOLE_LOG_TRACE(...)   Bolt::Log::GetEditorConsoleLogger()->trace(__VA_ARGS__)
#define BT_CONSOLE_LOG_INFO(...)    Bolt::Log::GetEditorConsoleLogger()->info(__VA_ARGS__)
#define BT_CONSOLE_LOG_WARN(...)    Bolt::Log::GetEditorConsoleLogger()->warn(__VA_ARGS__)
#define BT_CONSOLE_LOG_ERROR(...)   Bolt::Log::GetEditorConsoleLogger()->error(__VA_ARGS__)
#define BT_CONSOLE_LOG_FATAL(...)   Bolt::Log::GetEditorConsoleLogger()->critical(__VA_ARGS__)
...
namespace Bolt {
	...
	void Log::PrintAssertMessage(Log::Type type, std::string_view prefix, std::format_string<Args...> message, Args&&... args)
	{
		...
		MessageBoxA(nullptr, formatted.data(), "Bolt Assert", MB_OK | MB_ICONERROR);
#endif
	}
...
#if HZ_ASSERT_MESSAGE_BOX
		MessageBoxA(nullptr, "No message :(", "Bolt Assert", MB_OK | MB_ICONERROR);
#endif
	}
}
